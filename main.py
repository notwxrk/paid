...
def init_db():
    try:
        ...
        # Orders table - add delivery_cost column
        cur.execute('''
        ...
        # Check and add delivery_cost column if missing
        cur.execute("""
            SELECT column_name 
            FROM information_schema.columns 
        ...
        if not cur.fetchone():
            ...
            print("Added delivery_cost column to orders table")
            
        # Check and add final_price column if missing
        cur.execute("""
            SELECT column_name 
            FROM information_schema.columns 
            WHERE table_name='orders' AND column_name='final_price'
        """)
        if not cur.fetchone():
            cur.execute("ALTER TABLE orders ADD COLUMN final_price DECIMAL(10,2) NOT NULL DEFAULT 0")
            print("Added final_price column to orders table")
...
# Conversation states
WAITING_QUANTITY, WAITING_LOCATION, WAITING_ADDRESS, WAITING_PHONE, WAITING_PAYMENT_SCREENSHOT = range(5)
...
# Delivery cost for regions (outside Tashkent)
DELIVERY_COST = 25000
...
async def handle_quantity(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        # Ask for location
        await update.message.reply_text(
            "📍 Iltimos, lokatsiya yuboring yoki manzilingizni matn shaklida kiriting:\n\n"
            "🗺 Lokatsiya yuborish uchun pastdagi 'Lokatsiya yuborish' tugmasini bosing yoki "
            "oddiygina manzilingizni matn shaklida yozib yuboring.",
            reply_markup=InlineKeyboardMarkup([[
                InlineKeyboardButton("📍 Lokatsiya yuborish", request_location=True)
            ]])
        )
...
async def handle_location(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.message.location:
        ...
        # Tashkent coordinates approximation
        is_tashkent = (41.2 <= latitude <= 41.4) and (69.1 <= longitude <= 69.5)
        ...
        if is_tashkent:
            ...
            region = "Boshqa viloyat"
        
        context.user_data['delivery_cost'] = delivery_cost
        context.user_data['region'] = region
        context.user_data['location'] = f"{latitude}, {longitude}"
        
        final_price = context.user_data['total_price'] + delivery_cost
        context.user_data['final_price'] = final_price
        
        # Show delivery info
        if delivery_cost == 0:
            delivery_info = "🚖 Yetkazib berish: BEPUL (Toshkent)"
        else:
            delivery_info = f"🚖 Yetkazib berish: {delivery_cost:,} UZS ({region})"
        
        await update.message.reply_text(
            f"📍 Sizning joylashuvingiz: {region}\n"
            f"{delivery_info}\n"
            f"💰 Mahsulot narxi: {context.user_data['total_price']:,.0f} UZS\n"
            f"💵 Jami summa: {final_price:,.0f} UZS\n\n"
            f"🏠 Iltimos, aniq manzilingizni kiriting (ko'cha, uy, kvartira):"
        )
        return WAITING_ADDRESS
    else:
        await update.message.reply_text("❌ Iltimos, lokatsiya yuboring yoki manzilingizni kiriting:")
        return WAITING_LOCATION

async def handle_address(update: Update, context: ContextTypes.DEFAULT_TYPE):
    context.user_data['address'] = update.message.text
    
    # If location wasn't sent, assume it's outside Tashkent
    if 'delivery_cost' not in context.user_data:
        context.user_data['delivery_cost'] = DELIVERY_COST
        context.user_data['region'] = "Boshqa viloyat (manzil bo'yicha)"
        context.user_data['final_price'] = context.user_data['total_price'] + DELIVERY_COST
...
async def handle_phone(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if delivery_cost == 0:
        delivery_info = "🚖 Yetkazib berish: BEPUL"
    else:
        delivery_info = f"🚖 Yetkazib berish: {delivery_cost:,} UZS"
    
    keyboard = [
        [InlineKeyboardButton("💳 Click, Payme, Uzumbank", callback_data="sum")],
        [InlineKeyboardButton("СБЕР (RUBL)", callback_data="sber")],
        [InlineKeyboardButton("CRYPTO (USDT)", callback_data="crypto")]
    ]
    reply_markup = InlineKeyboardMarkup(keyboard)
...
async def ask_payment_details(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if payment_method == "sum":
        message = f"""
💳 Click, Payme, Uzumbank orqali to'lov:

💰 To'lov summasi: {final_price:,.0f} UZS
💳 Hisob raqam: `9860 0803 1478 1347`

💡 To'lov qilgach, skrinshot yuboring to'lov cheki.
        """
        await update.callback_query.message.reply_text(message, parse_mode='Markdown')
        context.user_data['waiting_screenshot'] = True
        
    elif payment_method == "sber":
        # Convert to RUBL (approximate rate)
        rub_amount = final_price / 150  # Approximate UZS to RUB rate
        message = f"""
💳 СБЕР Bank orqali to'lov:

💰 To'lov summasi: {rub_amount:.2f} RUBL
...
💡 To'lov qilgach, skrinshot yuboring to'lov cheki.
        ...
        usdt_amount = final_price / 12500  # Taxminiy kurs
        await update.callback_query.message.reply_text(
            f"🔗 To'lov tarmog'ini tanlang:\n💰 Jami summa: ${usdt_amount:.2f} USDT (taxminan)",
            reply_markup=reply_markup
        )
...
async def handle_payment_screenshot(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.message.photo:
        ...
        try:
            ...
            cur.execute('''
                INSERT INTO orders (user_id, product_id, quantity, total_price, delivery_cost, final_price, address, phone, payment_method, payment_screenshot)
            ...
            ''', (
                ...
                context.user_data.get('delivery_cost', 0),
                context.user_data['final_price'],
            ...
            # Notify admin
            delivery_info = "BEPUL (Toshkent)" if context.user_data.get('delivery_cost', 0) == 0 else f"{context.user_data.get('delivery_cost', 0):,} UZS"
...
📍 Hudud: {context.user_data.get('region', 'Noma\'lum')}
💰 Miqdor: {context.user_data['quantity']} ta
💵 Mahsulot narxi: {context.user_data['total_price']:,.0f} UZS
🚖 Yetkazib berish: {delivery_info}
💳 To'lov usuli: {context.user_data['payment_method']}
💎 Jami summa: {context.user_data['final_price']:,.0f} UZS
...
async def show_my_orders(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        for order in orders:
            ...
            delivery_info = "BEPUL" if order['delivery_cost'] == 0 else f"{order['delivery_cost']:,.0f} UZS"
            
            message = f"""
📦 Buyurtma #{order['id']}
🏷 Mahsulot: {order['product_name']}
💰 Miqdor: {order['quantity']} ta
💵 Mahsulot narxi: {order['total_price']:,.0f} UZS
🚖 Yetkazib berish: {delivery_info}
💎 Jami summa: {order['final_price']:,.0f} UZS
...
📍 Toshkent bo'ylab: 1 kun ichida - BEPUL
📍 O'zbekiston boshqa hududlari: 5 kun ichida - 25,000 UZS

💡 Buyurtma berishda lokatsiya yuborishingizni so'raymiz, 
shunda biz sizning joylashuvingizni aniqlab, yetkazib berish narxini avtomatik hisoblaymiz.
    """
    await update.callback_query.message.reply_text(message)

# Admin functions (unchanged from your original code)
...
# Add product conversation - FIXED VERSION with file_id
...
async def handle_product_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        if update.message.photo:
            ...
            print(f"Photo File ID: {photo_file_id}")  # Debug uchun
...
def main():
    ...
    order_conv_handler = ConversationHandler(
        ...
        states={
            ...
            WAITING_LOCATION: [MessageHandler(filters.LOCATION | filters.TEXT & ~filters.COMMAND, handle_location)],
