...
def init_db():
    try:
        ...
        if 'photo_url' not in existing_columns:
            missing_columns.append('photo_url')
        ...
        """)
        existing_orders_columns = [row['column_name'] for row in cur.fetchall()]
        
        orders_missing_columns = []
        if 'payment_screenshot' not in existing_orders_columns:
            orders_missing_columns.append('payment_screenshot')
        if 'status' not in existing_orders_columns:
            orders_missing_columns.append('status')
        if 'created_at' not in existing_orders_columns:
            orders_missing_columns.append('created_at')
        
        for column in orders_missing_columns:
            if column == 'payment_screenshot':
                cur.execute("ALTER TABLE orders ADD COLUMN payment_screenshot TEXT")
            elif column == 'status':
                cur.execute("ALTER TABLE orders ADD COLUMN status VARCHAR(50) DEFAULT 'pending'")
            elif column == 'created_at':
                cur.execute("ALTER TABLE orders ADD COLUMN created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP")
