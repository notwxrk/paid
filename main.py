...
💡 To'lov qilgach, skrinshot yuboring.
        ...
        reply_markup = InlineKeyboardMarkup(keyboard)
        context.user_data['crypto_step'] = 'select_network'
...
async def handle_product_quantity(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        await update.message.reply_text("🖼 Mahsulot rasmini yuboring (opsional):")
...
async def admin_orders(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        cur.execute('''
            ...
            FROM orders o 
            LEFT JOIN products p ON o.product_id = p.id 
            LEFT JOIN (SELECT DISTINCT user_id, username FROM (SELECT user_id, username FROM orders WHERE username IS NOT NULL) as tmp) u ON o.user_id = u.user_id
            ORDER BY o.created_at DESC
        ''')
        orders = cur.fetchall()
        cur.close()
        ...
        for order in orders:
            ...
            message = f"""
📦 Buyurtma #{order['id']}
👤 Foydalanuvchi: @{order['username'] or 'Noma\'lum'} (ID: {order['user_id']})
🏷 Mahsulot: {order['product_name']}
💰 Miqdor: {order['quantity']} ta
💵 Jami: {order['total_price']:,.0f} UZS
🏠 Manzil: {order['address']}
...
def main():
    ...
    application.add_handler(CallbackQueryHandler(handle_crypto_network, pattern="^(trc20|bsc20)$"))
