...
# PostgreSQL URL
POSTGRES_URL = "postgresql://dream_6s3v_user:vkAOuqwN79nizVnBlJddDgaeryO5PXaQ@dpg-d47l0sjipnbc73cv7vsg-a/dream_6s3v"
...
def save_product(name, description, price, quantity, image_file_id=None):
    ...
    query = '''
        ...
        VALUES (%s, %s, %s, %s, %s)
    '''
    return execute_query(query, (name, description, price, quantity, image_file_id))
...
async def show_my_orders(query, user_id):
    try:
        ...
        for order in orders:
            ...
            message_text += f"💰 {order[4]:,} UZS\n"
            message_text += f"📅 {order[10].strftime('%Y-%m-%d %H:%M')}\n"
            message_text += f"📊 Holati: {order[9]}\n\n"
...
⏰ Yetkazib berish muddati:
   • Toshkent shahri bo'ylab: 1 kun
   • Boshqa hududlar: 3 kun
...
async def handle_order_quantity(update, user_id, quantity_text):
    try:
        ...
        if quantity > product[4]:
            await update.message.reply_text(f"❌ Siz so'ragan miqdor mavjud emas! Maksimal: {product[4]} ta")
            return
    ...
    if state == PAYMENT_SCREENSHOT:
        ...
        if user_id == ADMIN_ID and 'new_product' in user_data.get(user_id, {}):
            ...
            if result:
                ...
                await update.message.reply_text(
                    "❌ Tovar qo'shishda xatolik. Iltimos, qayta urinib ko'ring.",
                    reply_markup=admin_menu_keyboard()
                )
...
async def handle_admin_add_product_quantity(update, user_id, quantity_text):
    try:
        ...
        await update.message.reply_text(
            "🖼️ Tovar uchun rasm yuboring (ixtiyoriy). Agar rasm yubormasangiz, 10 soniya kuting...",
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("Rasmsiz davom etish", callback_data="skip_image")]])
        )
        user_states[user_id] = ADMIN_ADD_PRODUCT_IMAGE
        
        # Agar 10 soniyada rasm yuborilmasa, rasmsiz saqlash
        async def save_without_image():
            await asyncio.sleep(10)
            if user_states.get(user_id) == ADMIN_ADD_PRODUCT_IMAGE:
                product_data = user_data[user_id]["new_product"]
                result = save_product(
                    product_data["name"],
                    product_data["description"],
        ...
        # Background task for auto-save without image
        import asyncio
        asyncio.create_task(save_without_image())
...
async def show_all_products_admin(query):
    try:
        ...
        for product in products:
            message_text += f"🧨 {product[1]}\n"
            message_text += f"💰 {product[3]:,} UZS\n"
...
async def confirm_order(query, order_id):
    try:
        ...
        if order:
            ...
            # Restore product quantity (order cancelled bo'lsa)
