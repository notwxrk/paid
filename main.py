...
async def calculate_and_update_income(user_id: int):
    ...
    conn = await init_db()
    try:
        # Faqat aktiv, muddati o'tmagan va 24 soat o'tgan mashinalarni olamiz
        cars = await conn.fetch(
            '''SELECT * FROM user_cars 
               WHERE user_id = $1 AND is_active = TRUE 
               AND expires_at > NOW() 
               AND (last_income_date IS NULL OR last_income_date <= NOW() - INTERVAL '24 hours')''',
            user_id
        )
        ...
        for car in cars:
            ...
            car_details.append({
                ...
                'next_income': car['last_income_date'] + timedelta(hours=24) if car['last_income_date'] else datetime.now() + timedelta(hours=24)
            })
        
        # Agar daromad bo'lsa, balansga qo'shamiz va last_income_date ni yangilaymiz
        if total_income > 0:
            await conn.execute(
            ...
            # Har bir mashinaning last_income_date ni yangilaymiz
            for car in cars:
                await conn.execute(
            ...
            notifications.append(f"🎉 Daromad qo'shildi: {total_income:,.0f} so'm")
            logger.info(f"Auto income updated for user {user_id}: {total_income}")
        
        # Keyingi daromad vaqtini hisoblaymiz
        next_income_cars = await conn.fetch(
            '''SELECT * FROM user_cars 
               WHERE user_id = $1 AND is_active = TRUE 
               AND expires_at > NOW() 
               AND last_income_date > NOW() - INTERVAL '24 hours' ''',
            user_id
        )
        
        next_income_time = None
        for car in next_income_cars:
...
# Start command - TELEFON MUAMMOSI TUZATILGAN
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not user:
        # Ask for phone number - TUZATILGAN
...
# Referral section - YANGI TIZIM (5% bonus)
async def show_referral(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = (
        ...
        f"• Siz do'stingizning haridining 5% ini olasiz"
    )
