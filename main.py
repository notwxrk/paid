...
async def ask_quantity(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if 'selected_product' not in context.user_data:
        await update.callback_query.message.reply_text("❌ Xatolik: Mahsulot tanlanmagan. Iltimos, qaytadan urinib ko'ring.")
...
async def handle_quantity(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if 'selected_product' not in context.user_data:
        await update.message.reply_text("❌ Xatolik: Mahsulot tanlanmagan. Iltimos, qaytadan urinib ko'ring.")
        return ConversationHandler.END
...
async def handle_phone(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if 'selected_product' not in context.user_data or 'total_price' not in context.user_data:
        await update.message.reply_text("❌ Xatolik: Ma'lumotlar yo'qolgan. Iltimos, qaytadan urinib ko'ring.")
        context.user_data.clear()
        return ConversationHandler.END
...
async def ask_payment_details(update: Update, context: ContextTypes.DEFAULT_TYPE):
    required_keys = ['selected_product', 'total_price', 'total_with_delivery', 'delivery_region', 'delivery_cost']
    for key in required_keys:
        if key not in context.user_data:
            await update.callback_query.message.reply_text("❌ Xatolik: Ma'lumotlar yo'qolgan. Iltimos, qaytadan urinib ko'ring.")
            context.user_data.clear()
            return
...
async def handle_crypto_network(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if 'total_with_delivery' not in context.user_data:
        await query.message.reply_text("❌ Xatolik: Ma'lumotlar yo'qolgan. Iltimos, qaytadan urinib ko'ring.")
        context.user_data.clear()
        return
...
async def handle_payment_screenshot(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    required_keys = ['selected_product', 'quantity', 'total_price', 'address', 'phone', 'payment_method', 'delivery_region', 'delivery_cost']
    for key in required_keys:
        if key not in context.user_data:
            await update.message.reply_text("❌ Xatolik: Ma'lumotlar yo'qolgan. Iltimos, qaytadan urinib ko'ring.")
            context.user_data.clear()
    ...
    try:
        if update and update.effective_message:
            await update.effective_message.reply_text(
                "❌ Xatolik yuz berdi. Iltimos, /start ni bosing va qaytadan urinib ko'ring."
            )
    except Exception as e:
