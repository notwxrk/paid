...
async def create_user(user_id: int, phone_number: str, referred_by: int = None):
    ...
    try:
        ...
        # Referal bonus - faqat birinchi marta
        if referred_by:
            # Tekshiramiz, oldin referal bonus berilganmi
...
async def calculate_daily_income(user_id: int):
    ...
    if total_income > 0:
        ...
        try:
            ...
            logger.info(f"Daily income calculated for user {user_id}: {total_income}")
        except Exception as e:
            logger.error(f"Error updating total earned for user {user_id}: {e}")
        finally:
            await conn.close()
...
# Balance section - PUL YECHISH KNOPKASI QO'SHILGAN
...
async def withdraw_money(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    commission = user['balance'] * 0.15
    final_amount = user['balance'] - commission
...
# Referral section - FAQAT BIR MARTA BONUS
async def show_referral(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = (
        ...
        f"• Har bir taklif uchun: 1,000 so'm (faqat 1 marta)\n"
...
async def show_withdraw_requests(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        for req in requests:
            ...
            commission = req['amount'] * 0.15
            final_amount = req['amount'] - commission
        ...
        try:
            ...
            if action == 'approve':
                ...
                await context.bot.send_message(
                    ...
                    f"💰 {request['amount'] - commission:,.0f} so'm kartangizga o'tkazildi\n"
                    f"📉 Komissiya (15%): {commission:,.0f} so'm"
                )
