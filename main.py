...
Flask + Telegram bot (polling in background) for Render + UptimeRobot setup.

How it works:
- A small Flask web app exposes "/" and "/health" endpoints so Render/UptimeRobot can ping it and keep the service alive.
- On the first incoming HTTP request the bot polling is started in a background daemon thread (so it doesn't block the webserver).
- Bot logic: admin can add products with photo/title/price/qty/desc, users can view products and create orders (SBER/CRYPTO),
  upload payment screenshot; admin can approve/reject/delete orders. Uses PostgreSQL via SQLAlchemy.

Environment variables (set these on Render):
- BOT_TOKEN (required)
- DATABASE_URL (required) e.g. postgresql://user:pass@host:port/dbname
- ADMIN_ID (required, integer)
- USDT_TRC20_ADDR (optional)
- USDT_BSC20_ADDR (optional)
- DEFAULT_UZS_PER_USDT (optional, default 12000)

Run on Render with gunicorn (see render.yaml). UptimeRobot should ping the root URL every 5 minutes.
"""
...
DEFAULT_UZS_PER_USDT = Decimal(os.getenv('DEFAULT_UZS_PER_USDT', '12000'))
...
# ---------------- Bot logic ----------------
# Conversation states
ADD_TITLE, ADD_PRICE, ADD_QTY, ADD_DESC, ADD_PHOTO, ORDER_QTY, ORDER_ADDRESS, ORDER_PHONE, ORDER_PAYMENT_CHOICE, ORDER_CRYPTO_NET, ORDER_SCREENSHOT = range(11)

main_menu_kb = ReplyKeyboardMarkup([['🧨 Tovarlar', '✨️ Mening Buyurtmalarim'], ['🚖 Yetkazib Berish']], resize_keyboard=True)

# We'll create the Application inside the bot thread
BOT_APP = None

def run_bot_polling():
    \"\"\"Start telegram bot polling. This function runs in a daemon thread.\"\"\"
    global BOT_APP
    logger.info('Starting Telegram bot (polling) in background thread...')
    BOT_APP = ApplicationBuilder().token(BOT_TOKEN).build()

    # Handler definitions (simplified wrapper functions to access DB)
    async def start_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
        user = update.effective_user
        await update.message.reply_text(f\"Assalomu alaykum, {user.first_name}!\", reply_markup=main_menu_kb)

    async def delivery_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
        await update.message.reply_text(\"🚚 Yetkazib berish: O'zbekiston bo'ylab 3 kun ichida, bepul.\")
    ...
    async def my_orders_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            if not orders:
                await update.message.reply_text('Sizning buyurtmalaringiz mavjud emas.')
                return
            for o in orders:
                prod_title = o.product.title if o.product else 'Noma\\'lumot'
                text = (f\"📦 Buyurtma: {o.uid}\\nMahsulot: {prod_title}\\nMiqdor: {o.qty}\\nJami: {int(o.total_uzs):,} UZS\\nStatus: {o.status}\\nManzil: {o.address}\\nTelefon: {o.phone}\\nVaqt: {o.created_at.strftime('%Y-%m-%d %H:%M')}\")
    ...
    async def buy_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            if not prod:
                ...
                return
            if prod.quantity <= 0:
                await query.message.reply_text('Kechirasiz, mahsulot zaxirada yo\\'q.')
                return
    ...
    async def order_qty_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            if not prod:
                await update.message.reply_text('Mahsulot topilmadi. Iltimos qayta boshlang.')
    ...
    async def order_address_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        await update.message.reply_text(\"Aloqa telefon raqamini kiriting (misol +998901234567):\")
    ...
    async def order_phone_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        kb = InlineKeyboardMarkup([[InlineKeyboardButton('СБЕР', callback_data='pay_sber'), InlineKeyboardButton('CRYPTO', callback_data='pay_crypto')]])
    ...
    async def payment_choice_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            if data == 'pay_sber':
                ...
                await query.message.reply_text(f\"СБЕР karta: 2202208046692951\\nMahsulot: {prod.title}\\nMiqdor: {qty}\\nJami: {int(total_uzs):,} UZS\\n\\nIltimos to'lov skrinshotini yuboring.\")
                return
            else:
                kb = InlineKeyboardMarkup([[InlineKeyboardButton('USDT TRC20', callback_data='crypto_trc20')],[InlineKeyboardButton('USDT BSC20', callback_data='crypto_bsc20')]])
                await query.message.reply_text('Iltimos tarmoqni tanlang:', reply_markup=kb)
                return
        finally:
            session.close()

    async def crypto_net_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
        query = update.callback_query
        await query.answer()
        data = query.data or ''
        session = SessionLocal()
        try:
            ...
            rate_val = get_setting(session, 'uzs_per_usdt', None)
            rate = Decimal(rate_val) if rate_val is not None else DEFAULT_UZS_PER_USDT
            if rate == 0:
            ...
            await query.message.reply_text(f\"USDT {net} manzil: {addr}\\nJami: {int(total_uzs):,} UZS\\nYuborish kerak: {total_usdt} USDT\\n\\nTo'lov skrinshotini yuboring.\")
        finally:
            session.close()
    ...
    async def receive_screenshot(update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            session.commit()
            await update.message.reply_text(\"Sizning buyurtmangiz qabul qilindi. Adminlar tekshiradi. 3 kun ichida yetkazib beriladi.\")
            kb = InlineKeyboardMarkup([[InlineKeyboardButton('Tasdiqlash', callback_data=f'admin_approve_{order.id}'), InlineKeyboardButton('Rad etish', callback_data=f'admin_reject_{order.id}')],[InlineKeyboardButton('O\\'chirish', callback_data=f'admin_delete_{order.id}')]])
            admin_text = (f\"🔔 Yangi buyurtma!\\nID: {order.uid}\\nUser: {order.user_name} ({order.user_id})\\nMahsulot: {prod.title}\\nMiqdor: {order.qty}\\nJami: {int(order.total_uzs):,} UZS\\nTo'lov: {order.payment_method} {( '('+ (order.crypto_network or '') +')') if order.crypto_network else ''}\\nTelefon: {order.phone}\\nManzil: {order.address}\\nVaqt: {order.created_at.strftime('%Y-%m-%d %H:%M')}\")
            try:
                await BOT_APP.bot.send_photo(chat_id=ADMIN_ID, photo=file_id, caption=admin_text, reply_markup=kb)
            except Exception:
                await BOT_APP.bot.send_message(chat_id=ADMIN_ID, text=admin_text, reply_markup=kb)
        finally:
            session.close()
            context.user_data.clear()
    ...
    async def admin_action_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            if action == 'approve':
                ...
                session.commit()
                try:
                    await BOT_APP.bot.send_message(chat_id=order.user_id, text=f\"Buyurtmangiz tasdiqlandi. Buyurtma tayyorlanmoqda va 3 kun ichida yetkazib beriladi. (ID: {order.uid})\")
                except Exception:
                    pass
                await query.message.reply_text('Buyurtma tasdiqlandi.')
            elif action == 'reject':
                ...
                session.commit()
                try:
                    await BOT_APP.bot.send_message(chat_id=order.user_id, text=f\"Kechirasiz, buyurtmangiz rad etildi. (ID: {order.uid})\")
                except Exception:
                    pass
                ...
                session.commit()
                await query.message.reply_text('Buyurtma o\\'chirildi.')
        finally:
            session.close()
    ...
    async def admin_add_desc(update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        await update.message.reply_text('Endi mahsulot rasmi yuboring (rasm):')
        return ADD_PHOTO
    ...
    async def admin_add_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        session = SessionLocal()
        try:
            p = Product(title=context.user_data.get('new_title'), description=context.user_data.get('new_desc'), price_uzs=context.user_data.get('new_price'), quantity=context.user_data.get('new_qty'), photo_file_id=file_id)
            session.add(p)
            session.commit()
    ...
    BOT_APP.add_handler(MessageHandler(filters.TEXT & (~filters.COMMAND), lambda u,c: start_handler(u,c) if u.message.text in ['🧨 Tovarlar','✨️ Mening Buyurtmalarim','🚖 Yetkazib Berish'] else message_router(u,c)))
    BOT_APP.add_handler(CallbackQueryHandler(buy_callback, pattern=r'^buy_'))
    BOT_APP.add_handler(CallbackQueryHandler(payment_choice_cb, pattern=r'^pay_'))
    BOT_APP.add_handler(CallbackQueryHandler(crypto_net_cb, pattern=r'^crypto_'))
    BOT_APP.add_handler(MessageHandler(filters.Regex(r'^\d+$') & (~filters.COMMAND), order_qty_handler))
    BOT_APP.add_handler(MessageHandler(filters.PHOTO | filters.Document.IMAGE, receive_screenshot))
    BOT_APP.add_handler(CallbackQueryHandler(admin_action_cb, pattern=r'^admin_'))
    BOT_APP.add_handler(CommandHandler('admin_add_product', admin_add_start))
    BOT_APP.add_handler(ConversationHandler(entry_points=[CommandHandler('admin_add_product', admin_add_start)],
                                           states={ADD_TITLE: [MessageHandler(filters.TEXT & (~filters.COMMAND), admin_add_title)],
                                           ...
                                           fallbacks=[CommandHandler('cancel', admin_cancel)], allow_reentry=True))
    BOT_APP.add_handler(CommandHandler('admin_products', admin_list_products))
    BOT_APP.add_handler(CallbackQueryHandler(admin_prod_delete_cb, pattern=r'^prod_delete_'))
    BOT_APP.add_handler(CommandHandler('admin_orders', admin_list_orders))
    BOT_APP.add_handler(CommandHandler('admin_set_rate', admin_set_rate_start))
    BOT_APP.add_handler(ConversationHandler(entry_points=[CommandHandler('admin_set_rate', admin_set_rate_start)],
                                           states={ADD_PRICE: [MessageHandler(filters.TEXT & (~filters.COMMAND), admin_set_rate_finish)]},
                                           fallbacks=[CommandHandler('cancel', admin_cancel)], allow_reentry=True))

    # Start polling (blocking) inside this thread
    try:
        BOT_APP.run_polling()
    except Exception as e:
        logger.exception('Error running bot polling: %s', e)


# Helper router for simple text menu mapping
async def message_router(update: Update, context: ContextTypes.DEFAULT_TYPE):
    txt = update.message.text.strip()
    if txt == '🧨 Tovarlar':
        return await list_products_handler(update, context)
    if txt == '✨️ Mening Buyurtmalarim':
        return await my_orders_handler(update, context)
    if txt == '🚖 Yetkazib Berish':
        return await delivery_handler(update, context)
...
# Start bot on first request (so Render/Gunicorn worker doesn't start multiple polling threads)
_bot_thread = None
_bot_started = False
...
def start_bot_once():
    ...
    with _bot_lock:
        if _bot_started:
            return
        _bot_thread = threading.Thread(target=run_bot_polling, daemon=True, name='telegram-bot-thread')
        _bot_thread.start()
        _bot_started = True
        logger.info('Bot thread started from before_first_request.')

# Export WSGI app as 'app' for gunicorn
app = flask_app

# Allow running locally with `python main.py` (useful for dev)
if __name__ == '__main__':
    # If running directly, start bot thread now then run Flask dev server
    with _bot_lock:
        if not _bot_started:
            _bot_thread = threading.Thread(target=run_bot_polling, daemon=True, name='telegram-bot-thread')
            _bot_thread.start()
            _bot_started = True
            logger.info('Bot thread started from __main__.')
