...
# Admin functions - TUZATILGAN: /approve va /reject
...
async def handle_withdraw_action(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # TUZATILGAN: Argumentlarni to'g'ri tekshirish
    if not context.args or len(context.args) < 2:
        await update.message.reply_text("❌ Xato: Komanda format: `/approve id` yoki `/reject id`")
        return
    ...
    try:
        ...
        try:
            ...
            if action == 'approve':
                ...
                await update.message.reply_text("❌ Noma'lum amal: faqat `approve` yoki `reject` ishlatishingiz mumkin")
