...
async def check_subscription_required(update: Update, context: ContextTypes.DEFAULT_TYPE):
...
async def create_user(user_id: int, phone_number: str, referred_by: int = None):
    ...
    try:
        ...
        # YANGI: Referal bonus berish (500 so'm har bir taklif uchun)
        if referred_by:
                ...
                if referrals_count >= 12 and not user_data['has_received_tico_bonus']:
                    ...
                    try:
                        await context.bot.send_message(
                            referred_by,
                            "🎉 Tabriklaymiz! Siz 12 ta do'stingizni taklif qilganingiz uchun BEPUL Tico mashinasi sovg'a qilindi! 🚗"
                        )
                    except:
                        pass
                    
                    logger.info(f"Tico bonus given to user {referred_by} for 12 referrals")
                
                logger.info(f"Referral bonus paid: {referred_by} -> {user_id}: 500 so'm")
...
# Start command - YANGI: MAJBURIY OBUNA TEKSHIRISH
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # YANGI: Har doim kanal va guruh a'zoligini tekshirish
...
async def show_main_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if await check_subscription_required(update, context):
